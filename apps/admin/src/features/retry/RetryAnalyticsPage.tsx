...
export default function RetryAnalyticsPage() {
  ...
  const handleReplayClick = async () => {
    ...
    try {
      ...
      setStatusMsg(`Replay triggered with "${profile}" profile. Charts will update as jobs flow.`);
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-3 border-b pb-3 mb-4">
        <div className="font-semibold text-sm">Seed Traffic Profile (dev only)</div>

        <select
          className="border rounded px-2 py-1 text-sm"
          value={profile}
          onChange={(e) => setProfile(e.target.value as ReplayProfile)}
          disabled={isLoading}
        >
        ...
        {statusMsg && <span className="text-xs text-emerald-700 ml-2">{statusMsg}</span>}
        {errorMsg && <span className="text-xs text-red-600 ml-2">{errorMsg}</span>}
      </div>

      {/* TODO: plug charts + tables */}
